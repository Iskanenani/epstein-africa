...
export async function getStaticProps({ locale }) {
  ...
  const frAvailable = hasFrenchStaticPage("archive");
  ...
  const senderCount = db
    .prepare(
      "SELECT COUNT(DISTINCT sender) AS n FROM emails WHERE COALESCE(is_promotional, 0) = 0 AND sender IS NOT NULL"
    )
    .get().n;
...
export default function Archive({ emailCount, senderCount, countryCount, countries, locale, frAvailable }) {
  ...
  const localizedAsPath = getLocalizedPath(router.asPath || "/archive", locale);
  ...
  const pushFilters = useCallback(({ page = 1, country = currentCountry, search = searchInput } = {}) => {
    ...
    router.push({ pathname: "/archive", query }, undefined, { shallow: true });
  ...
  return (
    <>
      <Head>
        ...
        <link rel="canonical" href={getCanonicalUrl("/archive", locale)} />
        <meta property="og:title" content={copy.title} />
        <meta property="og:description" content={description} />
        <meta property="og:url" content={getCanonicalUrl("/archive", locale)} />
        <meta property="og:type" content="website" />
        <meta property="og:locale" content={getOgLocale(locale)} />
        <meta
          property="og:image"
          content={`${BASE}/api/og?title=${encodeURIComponent(copy.heading)}&subtitle=${encodeURIComponent(`${emailCount.toLocaleString()} ${copy.ogSubtitle}`)}`}
        />
        {frAvailable && locale === "en" && (
          <link rel="alternate" hrefLang="fr" href={getCanonicalUrl("/archive", "fr")} />
        )}
        {frAvailable && locale === "fr" && (
          <link rel="alternate" hrefLang="en" href={getCanonicalUrl("/archive", "en")} />
        )}
      </Head>
      ...
      <div className="container">
        <header className="archive-header">
          <div className="archive-kicker">▌ {copy.kicker}</div>
          <h1 className="archive-title">{copy.heading}</h1>
          <p className="archive-subtitle">{copy.subtitle}</p>
          <div className="archive-stats">
            ...
            <span>
              <strong>{senderCount.toLocaleString()}</strong>
              {copy.statSenders}
            </span>
            <span>
              ...
              {loading ? (
                ...
                emails.map((email) => (
                  ...
                  >
                    ...
                    <td className="col-subject">{email.subject || copy.noSubject}</td>
